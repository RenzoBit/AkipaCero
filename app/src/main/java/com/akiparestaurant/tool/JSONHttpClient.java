package com.akiparestaurant.tool;
...
public class JSONHttpClient {
	public <T> int PostObject(final String url, final T object, final Class<T> objectClass) {
		...
		try {
			...
			if (httpEntity != null) {
				...
				String resultString = convertStreamToString(inputStream).trim();
				resultString = resultString.substring(1, resultString.length() - 1);
				resultString = resultString.replace("\\", "");
				inputStream.close();
				return new GsonBuilder().create().fromJson(resultString, int.class);
			}
